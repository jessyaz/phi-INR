...
def run_epoch(inr, loader, alpha, optimizer, cfg, device, is_train=True):
...
def main(cfg: DictConfig):
    ...
    assert cfg.data.look_back_window + cfg.data.horizon == 240, (
        f"look_back + horizon = {cfg.data.look_back_window + cfg.data.horizon} != 240h"
    )
    print(f"[✓] Cohérence : look_back={cfg.data.look_back_window}, "
          f"horizon={cfg.data.horizon}, lstm_hidden={cfg.inr.lstm_hidden_dim}")
    # ─────────────────────────────────────────────────────────

    trainset = NZDataset(ROOT / cfg.data.train_parquet, mode='train', latent_dim=cfg.inr.latent_dim)
    valset   = NZDataset(ROOT / cfg.data.val_parquet,   mode='val',   latent_dim=cfg.inr.latent_dim,
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=cfg.optim.t_max) \
    ...
    with mlflow.start_run():
        ...
        for epoch in tqdm(range(cfg.optim.epochs), desc="Epochs"):
            tr, tr_p, tr_h = run_epoch(inr, train_loader, alpha, optimizer, cfg, device, is_train=True)
            va, va_p, va_h = run_epoch(inr, val_loader,   alpha, None,      cfg, device, is_train=False)
            ...
            if epoch % cfg.misc.log_every_n_epochs == 0:
                tqdm.write(
                    f"[{epoch:>4}] tr={tr:.4f} tr_p={tr_p:.4f} tr_h={tr_h:.4f} "
                    f"| va={va:.4f} va_p={va_p:.4f} va_h={va_h:.4f} | best={best_val:.4f}"
                )
